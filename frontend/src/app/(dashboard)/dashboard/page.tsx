...
import { DashboardService, type DashboardData, type AttendanceDataPoint, type RecentActivity, type UpcomingEvent } from "@/services/dashboard";
        ...
        if (response.success && response.data) {
          ...
          setStats(prevStats => prevStats.map(stat => {
            if (stat.title === "Total Members") {
              return {
                ...stat,
                value: data.statistics.total_members?.toString() || "0",
                description: `${data.statistics.total_members || 0} total members`
              };
            }
            if (stat.title === "Total Families") {
              return {
                ...stat,
                value: data.statistics.total_families?.toString() || "0",
                description: `${data.statistics.active_families || 0} active families`
              };
            }
            if (stat.title === "Active Groups") {
              return {
                ...stat,
                value: data.statistics.active_groups?.toString() || "0",
            ...
            if (stat.title === "Upcoming Events") {
              return {
                ...stat,
                value: data.statistics.upcoming_events?.toString() || "0",
        ...
        <div className="chart-container rounded-3xl shadow-xl p-8 col-span-2">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex space-x-2">
              <button 
                onClick={() => setAttendancePeriod('current')}
                className={`px-4 py-2 rounded-xl text-sm font-medium shadow-lg transition-all duration-200 ${
                  attendancePeriod === 'current' 
                    ? 'bg-gradient-to-r from-blue-100 to-blue-200 dark:from-blue-900/20 dark:to-blue-800/20 text-blue-600 dark:text-blue-400' 
                    : 'bg-gradient-to-r from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-700 text-gray-600 dark:text-gray-300'
                }`}
              >
                This Month
              </button>
              <button 
                onClick={() => setAttendancePeriod('previous')}
                className={`px-4 py-2 rounded-xl text-sm font-medium shadow-lg transition-all duration-200 ${
                  attendancePeriod === 'previous' 
                    ? 'bg-gradient-to-r from-blue-100 to-blue-200 dark:from-blue-900/20 dark:to-blue-800/20 text-blue-600 dark:text-blue-400' 
                    : 'bg-gradient-to-r from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-700 text-gray-600 dark:text-gray-300'
                }`}
              >
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={attendanceData.length > 0 ? attendanceData : []}>
